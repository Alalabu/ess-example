...
const { ShiJi_Manager } = require('../../util/client-apps');
...
class AccessController extends Controller {
  ...
  async login() {
    ...
    const wxurl = `https://api.weixin.qq.com/sns/jscode2session?appid=${ShiJi_Manager.appid}&secret=${ShiJi_Manager.appsecret}&js_code=${loginCode}&grant_type=authorization_code`;
    ...
    try {
      ctx.body = await ctx.service.manager.access.login(openid, unionid, phonenum);
    } catch (error) {
