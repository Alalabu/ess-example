...
module.exports = app => {
  return {
    ...
    async task(ctx) {
      ...
      console.log('执行了定时器 auto_seller_done !', new Date());
      const order_done_at = new Calendar(); // 现在
      order_done_at.add(-30, CalendarTypes.MINUTES); // 当前时间 - 30min < seller_done_at ? 完成订单时机
      ...
      const orderList = await Order.findAll({
        ...
        where: {
          ...
          seller_done_at: {[ctx.Op.lt]: order_done_at.toDatetime()}, // 完成订单时机
        },
      });
      ...
      if (_.isArray(orderList)) {
        ...
        for (let x = 0; x < simpleOrderList.length; x++) {
          ...
          await ctx.service.seller.finish({
