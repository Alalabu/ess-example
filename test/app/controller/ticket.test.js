...
//       area_id: '93cfcc85967011ea84f700ff9846b53f', // 理工大曲江
//     };
//     const params = Object.keys(data).filter(k => data[k]).map(k => `${k}=${data[k]}`).join('&');
//     return app.httpRequest().get(`/ticket_info/findAll?${params}`).set(headers).then( res => {
//       console.log('/ticket_info/findAll => ', util.inspect(res.body, true, 4));
//     });
